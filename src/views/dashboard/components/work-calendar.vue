...
export default {
  ...
  props: {
    startDate: {
      type: Date,
      //  需要一个回调函数式的返回值
      default: () => new Date() // 如果没有传递starDate 就是当前时间
    }
  },
  data() {
    return {
      yearList: [], // 要遍历的年的数组
      currentYear: null, // 得到当前的年份
  ...
  created() {
    ...
    this.currentMonth = this.startDate.getMonth() + 1 // 当前的月份
    // 快速生成数组的方法
  ...
  methods: {
    dataChange() {
      // 生成新的日期
      this.currentDate = new Date(`${this.cueerntYear}-${this.currentMonth}-1`) // 以一号开始
    },
    // 判断当前是否式周末
    isWeek(date) {
